...
def save_data(df,db,tbl,lookup_key):
    ...
    conn.close()
def save_data_by_query(db,tbl,df):
    ...
    if tbl == 'drsend':
        ...
        try:
            ...
            for count, row in df.iterrows():  # enumerate(df1, start=1):  # reading all other rows
                ...
                query = query.format(','.join(columns), ','.join(
                    '?' * len(columns)))  # = column names, followed by same number of '?'
                ...
                conn1.commit()
                #st.info('DB updated. No errors found for', vslName)
